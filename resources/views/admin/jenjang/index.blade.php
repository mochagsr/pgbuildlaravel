...
@section('page-title', 'Kelola Jenjang')
@section('page-subtitle', 'Atur pilihan jenjang untuk produk (mis. SD, MI, SMP, MTs)')
...
<div class="bg-white rounded-xl shadow overflow-hidden">
    <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
                <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Nama Jenjang</th>
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50 transition">
                <td class="px-5 py-3 font-medium text-gray-800">{{ $jenjang->nama }}</td>
                <td class="px-5 py-3 text-center text-gray-600">{{ $jenjang->urutan }}</td>
                <td class="px-5 py-3 text-center">
                    <span class="bg-blue-100 text-blue-700 text-xs font-semibold px-2 py-1 rounded-full">
                        {{ $counts[$jenjang->nama] ?? 0 }}
                    </span>
                </td>
                <td class="px-5 py-3 text-center">
                    <div class="flex items-center justify-center gap-2">
                        <a href="{{ route('admin.jenjang.edit', $jenjang) }}"
                           class="w-8 h-8 bg-blue-100 text-blue-700 rounded-lg flex items-center justify-center hover:bg-blue-200 transition"
                           title="Edit">
                            <i class="fas fa-edit text-xs"></i>
                        </a>
                        <form method="POST" action="{{ route('admin.jenjang.destroy', $jenjang) }}"
                              onsubmit="return confirm('Hapus jenjang {{ $jenjang->nama }}?')">
                            @csrf @method('DELETE')
                            <button type="submit"
            ...
            <tr>
                <td colspan="4" class="px-5 py-12 text-center text-gray-400">
                    ...
                    Belum ada jenjang. <a href="{{ route('admin.jenjang.create') }}" class="text-blue-600 hover:underline">Tambah sekarang</a>
                </td>
            </tr>
            @endforelse
        </tbody>
